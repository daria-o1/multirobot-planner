/*
 * Date:      2020-11-10
 * Author:    Jan Faigl, Gaël Écorchard
 */
...
struct SMatrix {
  ...
  SMatrix(const SMatrix& a) : SIZE(a.NROWS* a.NCOLS), NROWS(a.NROWS), NCOLS(a.NCOLS) {
    _values = new T[SIZE];
  ...
  inline const T& operator()(int r, int c) const { return _values[r * NCOLS + c]; }

  inline void reset(T v) { for(int i = 0; i < SIZE; ++i) { _values[i] = v; } }
